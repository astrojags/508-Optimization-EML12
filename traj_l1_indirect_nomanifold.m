function traj_l1_indirect_nomanifold()
    % CR3BP trajectory optimization problem using indirect method without manifold targeting
    ...
    for i = 1:length(T_max_values)
        ...
        try
            ...
            [lambda0_sol, fval, exitflag] = fsolve(@(lambda0) shooting_function(lambda0, x0, m_0, ...
                xf_target, t0, tf, mu, curr_T_max, c), lambda0_guess, options);
    ...
    if solution_found
        ...
        [t, x, u, delta, switching] = integrate_trajectory(x0, m_0, lambda0_sol, t0, tf, mu, T_max, c);
...
function error = shooting_function(lambda0, x0, m0, xf_target, t0, tf, mu, T_max, c)
    ...
    [~, Z] = ode45(@(t, z) cr3bp_state_costate_dynamics(t, z, mu, T_max, c), [t0, tf], Z0);
...
function dZdt = cr3bp_state_costate_dynamics(t, Z, mu, T_max, c)
...
function [t, X, u, delta, switching] = integrate_trajectory(x0, m0, lambda0, t0, tf, mu, T_max, c)
    ...
    tspan = [t0, tf];
    while tspan(1) < tf
        [t_seg, Z_seg, te, ze, ie] = ode45(@(t, z) cr3bp_state_costate_dynamics(t, z, mu, T_max, c), tspan, Z0, options);
...
function [value, isterminal, direction] = switching_event(t, z, mu, T_max, c)
